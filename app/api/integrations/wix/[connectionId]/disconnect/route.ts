...
import { z } from "zod";
import { ComposioWixPublisher } from "@/lib/publishing/composio-wix-publisher";
...
export async function POST(request: Request, { params }: { params: Promise<{ connectionId: string }> }) {
  try {
    if (!bodySchema.safeParse(await request.json().catch(() => null)).success) {
      return NextResponse.json({ error: "Confirm before disconnecting Wix." }, { status: 400 });
    }
    const { connectionId } = await params;
    await new ComposioWixPublisher().disconnect(connectionId);
