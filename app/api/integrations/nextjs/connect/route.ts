...
import { connectNextJs } from "@/lib/publishing/nextjs/nextjs-connections";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await connectNextJs(input.data);
    return NextResponse.json({ ...result, message: "Next.js connected and verified." }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
