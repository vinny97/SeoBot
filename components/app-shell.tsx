...
import { Activity, ChevronDown, Compass, FileText, Globe, Home, Menu, Plug, Search, Settings, Sparkles, Users, X } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[248px_minmax(0,1fr)]">
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-[280px] flex-col border-r border-[var(--border)] bg-[#eef0eb] transition-[transform,visibility] lg:visible lg:sticky lg:top-0 lg:h-screen lg:w-auto lg:translate-x-0", open ? "visible translate-x-0" : "invisible -translate-x-full")}>
      ...
      <div className="border-t border-[var(--border)] p-4"><div className="flex w-full items-center gap-3 rounded-xl p-2" aria-label="User menu placeholder"><span className="grid h-9 w-9 place-items-center rounded-full bg-[var(--foreground)] text-sm font-semibold text-white">DE</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">Demo owner</span><span className="block truncate text-xs text-[var(--muted)]">Local workspace</span></span><ChevronDown size={15}/></div></div>
    </aside>
